...
if echo "$FILE_PATH" | grep -qE "\.(java|kt|ts|tsx|jsx|dart|sql)$"; then
  mkdir -p "$REPO_ROOT/.codex/runtime"
  echo "$FILE_PATH" >> "$REPO_ROOT/.codex/runtime/changed-files.txt"
  sort -u "$REPO_ROOT/.codex/runtime/changed-files.txt" -o "$REPO_ROOT/.codex/runtime/changed-files.txt"
fi
exit 0
