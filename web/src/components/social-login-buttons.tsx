  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex flex-col gap-3">
        {/* ZEOM-23: Kakao/Naver brand 색은 공식 컴플라이언스 토큰으로 유지 */}
        <button
          type="button"
          onClick={() => handleSocialLogin('kakao')}
          disabled={socialLoading !== null}
          className="w-full min-h-[44px] rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-opacity disabled:opacity-50 bg-brand-kakao text-brand-kakao-fg"
        >
        ...
        <button
          ...
          disabled={socialLoading !== null}
          className="w-full min-h-[44px] rounded-xl font-bold text-sm flex items-center justify-center gap-2 transition-opacity disabled:opacity-50 bg-brand-naver text-brand-naver-fg"
        >
