...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-flyway-migrations` | Flyway DB 마이그레이션과 JPA Entity 일관성 검증 | `backend/**/db/migration/V*.sql`, `backend/**/*Entity.java` |
| `verify-sendbird-videocall` | Sendbird 화상통화 파이프라인 검증 | `backend/**/sendbird/*.java`, `backend/**/consultation/*.java`, `web/**/consultation/**`, `web/**/counselor/room/**` |
| `verify-payment-wallet` | 결제/지갑/크레딧 시스템 무결성 검증 | `backend/**/wallet/*.java`, `backend/**/cash/*.java`, `backend/**/portone/*.java`, `backend/**/refund/*.java`, `backend/**/credit/*.java` |
...
| `verify-fortune` | 운세 엔진 도메인 무결성 검증 (사주 기반 + SHA-256 폴백) | `backend/**/fortune/**/*.java`, `web/src/app/fortune/**`, `web/src/app/my-saju/**`, `web/src/components/fortune-card.tsx`, `app_flutter/**/fortune/*.dart` |
