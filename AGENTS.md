...
| 문서 | 참조 시점 | 경로 |
|------|----------|------|
| Backend API | API 엔드포인트 추가/수정 시 | `.codex/docs/reference/backend-api.md` |
| Service Layer | 도메인 서비스/트랜잭션/예외 작업 시 | `.codex/docs/reference/service-layer.md` |
| Provider Integration | 외부 SDK 통합 (fake/real) 작업 시 | `.codex/docs/reference/provider-integration.md` |
| Sendbird Guide | 채팅/화상통화 작업 시 | `.codex/docs/reference/sendbird-guide.md` |
| Database & Migrations | 스키마/마이그레이션 작업 시 | `.codex/docs/reference/database-migrations.md` |
| Security Checklist | 인증/admin/CORS/멱등성 작업 시 | `.codex/docs/reference/security-checklist.md` |
| Frontend Pages | 페이지/라우팅 작업 시 | `.codex/docs/reference/frontend-pages.md` |
| Design System | UI 컴포넌트/색상/폰트 작업 시 | `.codex/docs/reference/design-system.md` |
| Flutter Architecture | Flutter feature/라우팅 작업 시 | `.codex/docs/reference/flutter-architecture.md` |
| Testing | 테스트 작성/수정 시 | `.codex/docs/reference/testing.md` |
| Environment | 환경 변수/배포 설정 시 | `.codex/docs/reference/environment.md` |
...
검증 스킬 (`.agents/skills/verify-*/SKILL.md`):
...
### 운영 모드
- `HARNESS_MODE`는 `.codex/hooks/*.sh`의 `env` 참조 (현재 `auto`)
...
- Sprint Contract: `.codex/runtime/sprint-contract/{ISSUE-KEY}.md`
- Verdict: `.codex/runtime/aggregate-verdict.md`
- State: `.codex/runtime/workflow-state.json`
- Metrics: `.codex/runtime/harness-metrics/scorecard.md` (집계: `bash .codex/runtime/harness-metrics/aggregate.sh`)
...
2026-04-28: Harness Engineering Integration 섹션 추가(에이전트 디스패치·아티팩트 경로). hook 명령 절대경로화(`Codex hook scripts`)
